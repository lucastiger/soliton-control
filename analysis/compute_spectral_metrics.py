#!/usr/bin/env python
"""Compute the single-snapshot spectral metrics of the validated single DKS.

This is the thin CLI entry point for the metrics in
:mod:`analysis.spectral_metrics`.  It loads the committed SINGLE-SNAPSHOT
...
``analysis.dks_access`` -- never hardcoded.  At the production operating point
(delta_omega = 10 kappa, past the ~9.3-9.4 kappa Hopf edge) the DKS is
stationary, so the committed input is a single-snapshot ``|a_mu|**2`` spectrum
(a reproducible observable in its own right); this script only consumes that
npz -- no averaging or other logic happens here.
"""
...
def _load_spectrum(npz_path: Path) -> dict:
    """Load a single-snapshot spectrum npz into ``mu`` and linear power ``P_mu``.
